...
class MetricHarness:
  ...
  def __call__(self, rgb_pred, rgb_gt, name_fn=lambda s: s):
    ...
    if rgb_pred.shape[-1] != rgb_gt.shape[-1]:
      # Assuming rgb_gt might have an extra alpha channel
      if rgb_gt.shape[-1] == 4:
        rgb_gt = rgb_gt[..., :3]  # Drop the alpha channel
      # If rgb_pred has an extra channel instead
      elif rgb_pred.shape[-1] == 4:
        rgb_pred = rgb_pred[..., :3]  # Drop the alpha channel

    # Ensure images are compatible for subtraction
    if rgb_pred.shape != rgb_gt.shape:
      raise ValueError("The shapes of rgb_pred and rgb_gt are incompatible for operations.")
    #*******************************************************************************************
