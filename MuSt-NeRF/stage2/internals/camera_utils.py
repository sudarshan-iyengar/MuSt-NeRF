...
def create_render_spline_path(
  ...
  if utils.isdir(config.render_spline_keyframes):
    ...
    keyframe_names = utils.listdir(config.render_spline_keyframes)
  else:
    ...
    with utils.open_file(config.render_spline_keyframes, 'r') as fp:
      ...
      keyframe_names = fp.read().splitlines()
  ...
  render_poses = generate_interpolated_path(
      ...
      rot_weight=.1)
  #***********************************************************************************************************************   
  print("*****************************************REACHEDDD CAMERA UTILS: printing the camera poses form the spline************************************************")
  counter = 1
  out_dir = config.checkpoint_dir
  file_path = os.path.join(out_dir, 'tv_spline_pose.json')
  with open(file_path, 'w') as f:
  
      for pose in render_poses:
          #pose_with_index = f"{counter}: {pose}\n\n"
          #print("Printing pose =" + str(counter))
          #f.write(pose_with_index)
          #counter +=1
          #f.write(str(pose) + "\n")
          #print(pose) 
          formatted_pose = f"P_mipnerf_{counter:04} = np.array([\n"
          for row in pose:
              formatted_pose += f"    [{', '.join(f'{num:.16f}' for num in row)}],\n"
          formatted_pose += "])\n\n"
          f.write(formatted_pose)
          counter += 1
  #***********************************************************************************************************************   
